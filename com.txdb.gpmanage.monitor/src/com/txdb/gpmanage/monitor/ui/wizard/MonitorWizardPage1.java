...
public class MonitorWizardPage1 extends BaseMonitorWizardPage {
	...
	protected MonitorWizardPage1() {
		super("Monitor Server Configuration", ResourceHandler.getValue(MessageConstants.WIZARD_PAGE1_TITLE));
	}
	...
	public void createBody(Composite parent) {
		...
		operateGroup.setText(ResourceHandler.getValue(MessageConstants.WIZARD_PAGE1_GROUP_HOSTCFG));
		operateGroup.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
		operateGroup.setLayout(new GridLayout(2, false));
		
		new Label(operateGroup, SWT.NONE).setText(ResourceHandler.getValue(MessageConstants.WIZARD_PAGE1_GROUP_HOSTCFG_HOSTNAME));
		txt_hostname = new Text(operateGroup, SWT.BORDER);
		txt_hostname.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
		
		new Label(operateGroup, SWT.NONE).setText(ResourceHandler.getValue(MessageConstants.WIZARD_PAGE1_GROUP_HOSTCFG_PORT));
		...
		sshGroup.setText(ResourceHandler.getValue(MessageConstants.WIZARD_PAGE1_GROUP_SSHCFG));
		sshGroup.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
		sshGroup.setLayout(new GridLayout(2, false));
		
		new Label(sshGroup, SWT.NONE).setText(ResourceHandler.getValue(MessageConstants.WIZARD_PAGE1_GROUP_SSHCFG_USERNAME));
		txt_username = new Text(sshGroup, SWT.BORDER);
		txt_username.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
		
		new Label(sshGroup, SWT.NONE).setText(ResourceHandler.getValue(MessageConstants.WIZARD_PAGE1_GROUP_SSHCFG_PASSWORD));
		...
		btn_savePwd.setText(ResourceHandler.getValue(MessageConstants.WIZARD_PAGE1_GROUP_SSHCFG_SAVEPWD));
		...
		btn_testConnection.setText(ResourceHandler.getValue(MessageConstants.WIZARD_PAGE1_GROUP_TESTCONN));
		...
		setMessage("Config a new Monitor Server", INFORMATION);
	}
	...
	private void updateWizardMsg() {
		...
		String wizardMsg = "Config a new Monitor Server";
	...
	public void taskComplete(Map<String, Object> taskMap) {
		...
		if (acceptNext) {
			...
			((MonitorWizard) getWizard()).setGpcontroller(gpController);
		...
		((BaseMonitorWizardPage) nextPage).restorePage();
