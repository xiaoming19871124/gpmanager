...
import com.txdb.gpmanage.install.ui.provider.InstallSegmentTableLabelProvider;
 ...
 *
 */
public class InstallSegmentComposite extends IupperComposite {
	private TableViewer checkboxTableViewer;
	private Label descLb;
    private boolean isFinish=false;
	public InstallSegmentComposite(InstallComposite mianComposite, Composite parent, CompositeCode code) {
		super(mianComposite, parent, code, ResourceHandler.getValue("install.segment.title"), ResourceHandler.getValue("install.segment.desc"));
	}
	...
	private void createCenterComposite(Composite mainComposite) {
		...
		installBtn.setLayoutData(new GridData(SWT.LEFT, SWT.TOP, false, false, 4, 1));
		...
		isSuccessColumn.setText(ResourceHandler.getValue("install.segment.isInstall"));
		checkboxTableViewer.setContentProvider(new ArrayContentProvider()); // 内容器
		checkboxTableViewer.setLabelProvider(new InstallSegmentTableLabelProvider());// 标签器
		...
		installBtn.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				boolean previous =((InstallComposite)mianComposite).getSsh().isFinish();
				if(!previous){
					text.append(ResourceHandler.getValue("installSegment.previous.error"));
					return;
				}
				isFinish=false;
				...
				new Thread(new Runnable() {
					@Override
					public void run() {
						isFinish =((InstallUiService) getService()).installSegment(info, text, InstallSegmentComposite.this);
